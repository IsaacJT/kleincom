...
kleincom - minimal dumb-terminal emulation program


# SYNOPSIS

**kleincom** [ _options_ ] _device_


# DESCRIPTION

As its name suggests, **kleincom(1)** is a minimal dumb-terminal
...
In effect, kleincom is not an "emulator" per-se. It is a simple program
...
When kleincom starts it opens the tty (serial port) given as its
...
just opened. Following this, if standard input is a tty, kleincom sets
...
copied to the serial port. kleincom also scans its input stream for a
...
character"). Depending on the value of the function character, kleincom
...
Commands are given to kleincom by first keying the *espace character*
...
:   Quit the program *without* resetting the serial port to its
    original settings. Terminating with the Quit command, kleincom
...
:   Write hex. kleincom prompts the user for a string of hexadecimal
...
kleincom accepts the following command-line options.
...
:   Defines the character that will make kleincom enter command-mode
    (see description above). If **x** is given, then **C-x** will make
    kleincom enter command mode. See also the **--no-escape**
    option. (Default: **a**)

**--no-escape** | **-n**

:   Disables the escape character. kleincom will never enter
    command-mode if this option is given. To exit kleincom, in this
...
:   If given, kleincom will not initialize, configure, or otherwise
    mess with the serial port at start-up. It will just open it. This
    is useful, for example, for connecting kleincom to
...
:   If given, kleincom will not reset the serial port when exiting. It
    ...
    kleincom. Regardless whether the **--noreset** option is given, the
    user can exit kleincom using the "Quit" command (instead of
    "Exit"), which makes kleincom behave *exactly* as if **--noreset**
    was given. See also the **--hangup** option. (Default: Disabled)

    NOTICE: kleincom clears the modem control lines on exit by setting
    the *HUPCL* control bit of the respective port. kleincom always
    ...
    kleincom. If **--noreset** is *not* given, or if both **--noreset**
    and **--hangup** are given, then HUPCL is set for the port and
    will remain so after exiting kleincom. This is true, regardless of
    the way kleincom terminates (command, read zero-bytes from standard
    input, killed by signal, fatal error, etc), and regardless of the
    **--noinit** option.

**--hangup** | **-u**

:   If given together with **--noreset**, kleincom will not reset the
    ...
    effect (without **--noreset** kleincom always clears the modem
    control lines on exit, anyway).

**--nolock** | **-l**

:   If given, kleincom will _not_ attempt to lock the serial port
    before opening it. Normally, depending on how it's compiled,
    kleincom attempts to get a UUCP-style lock-file
    ...
    is possible that your kleincom binary is compiled without support
...
:   Send the provided string after opening and configuring the serial
    ...
    terminal. Sending the init string, kleincom observes the **--omap**
...
:   Exit kleincom if it remains idle for the specified time (in
    milliseconds). kleincom is considered idle if: Nothing is read
    (received) from the serial port, AND there is nothing to write
    (send) to the serial port, AND nothing is read from the standard
    input (terminal). If **--exit-after** is set to zero, then kleincom
    ...
    kleincom drains the O/S serial port output buffer (i.e. waits for
    ...
    there has been closed), will *not* cause kleincom to exit. Instead,
    kleincom will keep running, *without* reading from stdin, and will
    ...
    zero bytes from the standard input causes kleincom to exit, after
    the contents of its output queue have been transmitted.

**--exit** | **-X**

:    Exit kleincom immediately after opening and configuring the serial
     ...
     (and possibly **--hangup**) kleincom can be used as a very crude
     replacement of **stty(1)**. If an init string is also given (see
     **--initstring** option), kleincom exits imediatelly after sending
...
:   kleincom will wait until the specified serial port is available, rather
    than exiting immediately if the port was not found.

**--quiet** | **-q**

:   Forces kleincom to be quiet. Suppresses the output of the initial
...
:   Print a short help message describing the command-line
    options. kleincom's version, compile-time options, and enabled
...
were changed) for the respective options. If kleincom determines that
...
kleincom). If the option and the corresponding serial-port setting are
the same, only a single value is shown. Example:

    *** baud: 9600

This behavior was introduced in kleincom 2.0. Older releases displayed
...
kleincom can send and receive files over the serial port using external
...
a kleincom session that uses **sz(1)** to transmit files, and **rz(1)**
to receive files, you have to say something like this:

    kleincom --send-cmd "sz -vv" --receive-cmd "rz -vv" ...
...
and the "receive" commands you can invoke kleincom like this:

    kleincom --send-cmd '' --receive-cmd '' ...

A kleincom session with both, the send- and the receive-file commands
disabled does not **fork(2)** and does not run any external programs.

During the kleincom session, if you key the "send" or "receive"
...
completion are available at this prompt, if you have compiled kleincom
...
kleincom operation. After entering a filename (and / or additional
transmission or reception program arguments) and assuming you have not
canceled the operation by pressing **C-c**, kleincom will start the
...
ignore SIGINT), and return control to kleincom. Pressing **C-c** at any
...
kleincom map (translate, replace) certain special characters after being
...
For example the command:
    ...
    kleincom --omap crlf,delbs --imap ignlf,bsdel --emap crcrlf ...
...
This section summarizes the conditions in which kleincom terminates its
...
- The exit command is seen in the standard input. That is, the escape
  ...
  options. After that kleincom exits with a success status.
...
- The quit command is seen in the standard input. That is, the escape
  ...
  that of the exit command, with one difference: kleincom behaves as if
  the **--noreset** option is given (regardless if it actually is, or
  not).

- The **--exit** option is given. See the documentation of this option
  for a description of what exactly happens in this case. kleincom
...
- The **--exit-after** option is given. See the documentation of this
  ...
  case. kleincom exits with a success exit status.

- Zero bytes are read from the standard input. This usually means that
  whatever was connected to kleincom's standard input has been closed
  or, if a file was connected, then kleincom has read up to the end of
  the file. In this case, if the **--exit-after** option is *not*
  given, kleincom stops reading from the standard input, and keeps
  operating normally (i.e. writing to, and reading from, the serial
  port) until its output queue empties. When this happens, kleincom
  ...
  again, kleincom stops reading from the standard input and continues
  operating normally but, in this case, it does so until it becomes
  idle for the specified amount of time, before exiting. kleincom exits
  with a success exit status.

- kleincom is killed by the TERM or INT signal, or an unrecoverable
  error occurs. In this case kleincom behaves as if it had received the
  ...
  modem control lines are cleared, and kleincom exits with a failure
  status.
...
<https://github.com/npat-efault/kleincom/releases>
...
This file is part of kleincom.

kleincom is free software; you can redistribute it and/or modify it
under the terms of the GNU General Public License as published by the
Free Software Foundation; either version 2 of the License, or (at your
option) any later version.

kleincom is distributed in the hope that it will be useful, but WITHOUT
